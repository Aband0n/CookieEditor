...
function setOptions() {
  ...
  for (var i = 0; i < existingLocales.length; i++) {
    $("#customLocale").append(
      $("<option>")
        .attr("value", existingLocales[i].code)
        .prop("selected", existingLocales[i].code == preferences.customLocale)
        .text(existingLocales[i].name)
    );
  }
  ...
  $("input:radio[value='" + preferences.maxCookieAgeType + "']").prop(
    "checked",
    true
  );
  $("#maxDateType").buttonset("refresh");

  $("option[value='" + preferences.copyCookiesType + "']").prop(
    "selected",
    true
  );
  ...
  else $("#showDomainBeforeNameLabel").removeClass("disabled");

  $("option[value='" + preferences.sortCookiesType + "']").prop(
    "selected",
    true
  );
...
function setEvents() {
  ...
  $("#showFlagAndDeleteAll").click(function() {
    preferences.showFlagAndDeleteAll = $("#showFlagAndDeleteAll").prop(
      "checked"
    );
  });
  ...
  $("#showDomainBeforeName").click(function() {
    preferences.showDomainBeforeName = $("#showDomainBeforeName").prop(
      "checked"
    );
  });
...
function updateMaxDate(filterAllCookies) {
  ...
  if (!tmp_useMaxCookieAge) {
    ...
    $("#maxDateType").buttonset("enable");
    $("#saveMaxDateButton").button("enable");
    if (!filterAllCookies) $("#saveMaxDateButton:hidden").fadeIn();
  }

  if (!tmp_useMaxCookieAge || filterAllCookies)
    preferences.useMaxCookieAge = tmp_useMaxCookieAge;

  if (filterAllCookies == undefined || filterAllCookies == false) return;

  preferences.maxCookieAgeType = $(
    "input:radio[name='radioMaxDate']:checked"
  ).val();
...
function shortenCookies(cookies, callback) {
  ...
  $("span", "#shortenProgress").text(
    totalCookies - cookies.length + " / " + totalCookies
  );
  ...
  if (
    ...
    cookie.expirationDate > maxAllowedExpiration
  ) {
    console.log(
      "Shortening life of cookie '" +
        cookie.name +
        "' from '" +
    ...
    cookiesShortened++;
  } else shortenCookies(cookies, callback);
}
