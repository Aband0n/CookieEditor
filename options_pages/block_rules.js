  ...
  $(".cmd_delete")
    ...
    .click(function() {
      if (
        ...
        confirm(_getMessage("Alert_deleteRule") + "?")
      ) {
        hideEditCommands();
...
function showNewEmptyRule() {
  if ($(".new_row:not(.template)").length > 0) {
    $(".new_rule_operations").css(
      "top",
      $(".new_row:not(.template)").position().top + "px"
    );
    $(".new_rule_operations").css(
      "left",
      $(".new_row:not(.template)").position().left + "px"
    );
    return;
  }
