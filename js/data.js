...
var ls = {
  ...
  get: function(name, default_value) {
    if (localStorage[name] === undefined) {
      if (default_value !== undefined) ls.set(name, default_value);
      else return null;
 ...
 */
function fetchData() {
  for (let key in preferences_template) {
    ...
    var onPreferenceChange = function(id, oldval, newval) {
      ...
      if (!syncTimeout) syncTimeout = setTimeout(syncDataToLS, syncTime);
      return newval;
    };
...
window.addEventListener(
  ...
  function(event) {
    try {
      ...
      var oldValue =
        event.oldValue !== null ? JSON.parse(event.oldValue) : null;
      var newValue =
      ...
      if (event.key.indexOf(preferences_prefix) === 0) {
        ...
        preferences[key] =
          newValue === null
            ? preferences_template[key].default_value
            : newValue;
        preferences_template[key].used = varUsed;
      } else if (event.key.indexOf(data_prefix) === 0) {
        key = event.key.substring(data_prefix.length);
        varUsed =
          data_template[key].used !== undefined && data_template[key].used;
        varChanged = data[key] !== newValue;
        data[key] =
          newValue === null ? data_template[key].default_value : newValue;
